<template>
    <div class="max-w-7xl mx-auto p-4">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">Table B - Transaksi</h1>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <vue-good-table :search-options="{ enabled: true, trigger: 'enter', placeholder: 'Search kode toko...' }"
        ...
        <BModal v-model="showModal" @update:model-value="closeModal"
            ...
            <form @submit.prevent="submitForm">
                ...
                <div class="mb-3">
                    ...
                    <input v-model.number="form.nominal_transaksi" type="number" step="0.01" class="form-control"
                        required />
                </div>
            </form>
        ...
        <BModal v-model="showDeleteModal" title="Delete Data">
            <div class="text-danger mb-4">
                Are you sure you want to delete <strong>Kode {{ dataToDelete?.kode_toko }}</strong>?
            </div>
            <template #footer>
        ...
        <!-- Modal Upload Excel -->
        <BModal v-model="showUploadModal" title="Upload Excel Table B">
...
export default {
    ...
    data() {
        return {
            ...
            columns: [
                ...
                { label: "Nominal Transaksi", field: "nominal_transaksi" },
            ...
            showModal: false,
            isEditMode: false,
            form: { kode_toko: "", nominal_transaksi: "" },
            errorMessage: "",
    ...
    methods: {
        async fetchData() {
            this.isLoading = true;
            try {
                const res = await apiClient.get("/table-b", {
                    params: {
                        search: this.search,
        ...
        openCreateModal() {
            ...
            this.form = { kode_toko: "", nominal_transaksi: "" };
            this.showModal = true;
        },
        openEditModal(row) {
        ...
        async submitForm() {
            try {
                if (this.isEditMode) {
                    await apiClient.put(`/table-b/${this.form.kode_toko}`, this.form);
                } else {
                    await apiClient.post("/table-b", this.form);
                }
                this.fetchData();
        ...
        async deleteConfirmed() {
            ...
            try {
                await apiClient.delete(`/table-b/${this.dataToDelete.kode_toko}`);
                this.fetchData();
        ...
        async submitUpload() {
            ...
            try {
                await apiClient.post("/table-b/upload-excel", formData, {
        ...
        async downloadExcel() {
            try {
                const res = await apiClient.get('/table-b/export-excel', {
                    params: {
                        search: this.search,
                        page: this.currentPage,
                        per_page: this.perPage,
                ...
                link.href = url;
                link.setAttribute('download', `table_b${timestamp}.xlsx`);
        ...
        async downloadPdf() {
            try {
                const res = await apiClient.get('/table-b/export-pdf', {
                    params: {
                        search: this.search,
                        page: this.currentPage,
                        per_page: this.perPage,
                ...
                const link = document.createElement('a');
                link.href = url;
                link.setAttribute('download', `table_b${timestamp}.pdf`);
                document.body.appendChild(link);
                link.click();
            } catch (err) {
                handleApiError(err);
